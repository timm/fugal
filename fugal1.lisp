...
; fugal1.lisp -- fugal.lisp on lib1.lisp: one accessor
; `?` (plist/hash/struct) and plist settings, so config
; is (? *settings* 'k) and cli is a two-liner. No reader
; macros; num/sym split via defmethod.
...
(defmethod mix ((i hash-table) j &optional (w 1))
  (let ((out (o)))
    (maphash (fn (incf (? out $1 0) $2)) i)
    (maphash (fn (incf (? out $1 0) (* w $2))) j)
    out))
...
(defmethod add+ ((i hash-table) v w) (incf (? i v 0) w) i)
...
;;; 3. discretization ------------------------------------------
(defmethod bin ((c num) v) (floor (* (? *settings* 'bins) (norm c v))))
...
(defmethod cuts-of ((c num) bins hi at)
  (let ((l (num)))
    (mapcar (fn (setf l (mix l (? bins $1)))
                (list at (- big) (? hi $1) l))
            (butlast (sort (keys bins) #'<)))))

(defmethod cuts-of ((c hash-table) bins hi at)
  (mapcar (fn (list at (? hi $1) (? hi $1) (? bins $1)))
          (keys bins)))
...
;;; 4. grow trees ----------------------------------------------
(defun mink (lst &optional (p (? *settings* 'p)))
  (let ((n (length lst)))
